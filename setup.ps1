# =====================================================================
# IT - 專業雲端部署 終極一鍵裝機總司令 二合一完美集成版 (修復版)
# =====================================================================
# 功能：權限獲取 電源調校 時間對時 三語部署 驅動 補丁更新 清理預載 8款智慧安裝 引導 + VPN
...
# 建立並強制寫入語言清單（將 English US 鎖定為第一順位預設顯示）
...
# 修正不相容參數：移除原本會導致紅字報錯的 -AcceptLicense，改用相容於 PowerShell 5.1 的寫法
Install-Module -Name PSWindowsUpdate -Force -SkipPublisherCheck -ErrorAction SilentlyContinue
Import-Module PSWindowsUpdate -ErrorAction SilentlyContinue

if (Get-Command Set-WUSettings -ErrorAction SilentlyContinue) {
    # 🎯【已修復】替換掉原本會報錯的參數，改用標準 ServiceID 註冊微軟更新（包含 Office、獨立驅動程式等）
...
# 2. Microsoft 365 (cn / en / tw) 強制移除
Write-Host " -> 正在安全檢索所有版本的 Microsoft 365 辦公套件..." -ForegroundColor Gray
# 從系統註冊表精準撈出所有叫 "Microsoft 365" 的軟體，但對排除 Windows Copilot 獨立應用
$o365InstalledList = Get-ItemProperty "HKLM:\Software\Microsoft\Windows\CurrentVersion\Uninstall\*", "HKLM:\Software\Wow6432Node\Microsoft\Windows\CurrentVersion\Uninstall\*", "HKCU:\Software\Microsoft\Windows\CurrentVersion\Uninstall\*" -ErrorAction SilentlyContinue | 
...
if ($copilotCheck) {
    Write-Host " -> [成功] Microsoft Copilot 安全保留：成功鎖定獨立應用，已安全跳過並留存本機！" -ForegroundColor Green
} else {
...
# 🎯【已修復】新增強制重置與更新 WinGet 來源的區塊，徹底根治 0x8a15000f 錯誤
Write-Host " -> 正在強制修復與重置 WinGet 本地快取來源..." -ForegroundColor Gray
...
foreach ($app in $apps) {
    ...
    if ($check -match $app) {
        ...
        # 強制走官方 winget 庫（避開 msstore 憑證出錯與錯誤代碼 0x8a15005e）
...
Write-Host " -> FortiClient 提示：由於官方限制較多，稍後將彈出介面供你手動點選下一步。" -ForegroundColor Yellow
...
Write-Host "大功告成！所有系統優化、補丁驅動更新、多語言與 8+1 款軟體配置完畢！" -ForegroundColor Green
Write-Host "=======================================================" -ForegroundColor Green
